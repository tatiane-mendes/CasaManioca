...
const applyFilters = (ingredients, filters) => ingredients.filter((ingredient) => {
  ...
  if (filters.hasAcceptedMarketing && !ingredient.hasAcceptedMarketing) {
    return false;
  }

  if (filters.isProspect && !ingredient.isProspect) {
    return false;
  }

  if (filters.isReturning && !ingredient.isReturning) {
    return false;
  }
...
const IngredientList = () => {
  ...
  const sortedIngredients = applySort(filteredIngredients, sort);
  const paginatedIngredients = applyPagination(sortedIngredients, page, rowsPerPage);
